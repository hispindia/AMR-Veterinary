...
export const getSterileTEI = async (
  ...
  var api_other = `../../../api/30/trackedEntityInstances/query.json?ou=${orgUnit}&ouMode=SELECTED&&order=created:desc&program=${sampleTestingProgram[1]}&programStatus=ACTIVE&&skipPaging=true`;
...
export const getAllTei = async (orgUnit, sampleTestingProgram, eventStatus) => {
  ...
  */
  ...
  // new API for event -- https://amr.hispindia.org/cskhphv_amr/api/30/trackedEntityInstances/query.json?ou=BGvNKwvkcJa&ouMode=SELECTED&&order=created:desc&program=WhYipXYg2Nh&programStatus=ACTIVE&eventOccurredAfter=2018-08-09&eventOccurredBefore=2024-01-30&programStage=LjiZPsbh1oy&skipPaging=true
  // change for cskhphv_amr
  ...
  api_GP3 = `../../../api/30/trackedEntityInstances/query.json?ou=${orgUnit}&ouMode=SELECTED&&order=created:desc&program=${sampleTestingProgram[2]}&programStatus=ACTIVE&skipPaging=true`;
  ...
  return axios
    ...
    .then(
      axios.spread((...responses) => {
        ...
        if (
          responseTwo.data ||
          responseThree.data || responseOne.data
        ) {
          responseOne.data.rows.forEach((responseOneTei) => {
            rowList.push(responseOneTei);
